// ── Subpage helpers ────────────────────────────────────────────────────
// @web-module-requires: model_generated, card_contract_generated, state, grid, config_option_core
...
function buttonConfigFields(b) {
  ...
  if (type === "") {
    ...
    options = b && b.options || "";
  } else if (type !== "action" && type !== "alarm_action" && !isClimateCardType(type) && type !== "cover" && type !== "garage" && type !== "gate" && type !== "webhook" && type !== "screen_lock" && type !== "media" && type !== "presence" && type !== "light_control" && type !== "fan_control" && type !== "solar" && type !== "ha_calendar" && !cardLargeNumbersSupported({ type: type, precision: precision })) {
    options = "";
  }
  if (type === "image") {
