...
inline ParsedCfg normalize_parsed_cfg(ParsedCfg p) {
  ...
  if (!p.type.empty() && p.type != "action" && p.type != "alarm" && p.type != "alarm_action" && p.type != "climate" && p.type != "garage" && p.type != "webhook" && p.type != "screen_lock" && p.type != "todo" && p.type != "sensor" && p.type != "door_window" && p.type != "presence" && p.type != "media" && p.type != "subpage" && p.type != "image" && p.type != "solar" && p.type != "ha_calendar" && !fan_card_type(p.type) && !card_large_numbers_supported(p)) {
    p.options.clear();
  }
  if (p.type == "sensor") {
